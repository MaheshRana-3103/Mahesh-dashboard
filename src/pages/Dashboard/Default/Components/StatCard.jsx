...
const StatCard = ({ title, value, increasedBy, decreasedBy }) => {
  return (
    <Link className="statCard" to={"/dashboard/projects/orders"}>
      <h3 className="statCardHeading semibold-14">{title}</h3>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8 }}
        className="statCardData"
      >
        <p className="semibold-24">{value}</p>
        {increasedBy && (
          <p className="statusView">
            <span className="regular-12 w-400">+{increasedBy}</span>
            <span>
              <svg
                width="16"
                height="16"
        ...
        {decreasedBy && (
          <p className="statusView">
            <span className="regular-12 w-400">-{decreasedBy}</span>
            <span>
              <svg
                width="16"
                height="16"
...
const StatCards = ({ statusCard }) => {
  return statusCard?.map((e, index) => {
    const { title, value, increasedBy, decreasedBy } = e || {};
    return (
