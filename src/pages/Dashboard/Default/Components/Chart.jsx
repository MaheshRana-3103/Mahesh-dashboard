...
export const RevenueChart = ({ LineData }) => {
  ...
  return (
    ...
    >
      {heading && (
        <div className="RChartHeadingContainer">
          <h3 className="semibold-14">{title}</h3>
          <ul className="RChartHeadingList">
            {list?.map((e, index) => {
              const { label, value } = e || {};
              return (
                <li key={index}>
...
export const BarProjectionChart = ({ BarData }) => {
  ...
  return (
    <motion.div
      ...
      className="chartContainer"
    >
      <div className="BChartHeadingContainer">
        <h3 className="semibold-14">{title}</h3>
      </div>
...
export const SalesChart = ({ data = [] }) => {
  ...
  return (
    <motion.div
      className="SalesChartContainer"
      initial="hidden"
      animate="visible"
      variants={containerVariants}
    >
      <motion.h2
        className={`$SalesChartH2} semibold-14`}
        variants={itemVariants}
      >
        Total Sales
      </motion.h2>

      <div className="SalesPieChart">
